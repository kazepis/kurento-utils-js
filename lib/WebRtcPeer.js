...
function WebRtcPeer(mode, options, callback) {
  ...
  if (!pc) {
	...
	pc.oniceconnectionstatechange = e => { 
      self.emit('iceconnectionstatechange', e.currentTarget.iceConnectionState);
    }									   
  }
...
function setMediaBitrates(sdp, videoBitrate = 0, audioBitrate = 0, googlemax = 0, googlemin = 0) {
  var codecId = getCodecId(sdp);
  var _maxSetting = "a=fmtp:"+ codecId +" x-google-max-bitrate=" + googlemax + "\r\n";
  var _minSetting = "a=fmtp:"+ codecId +" x-google-min-bitrate=" + googlemin + "\r\n";
  var _sdp = "";
  if (videoBitrate > 0) {
    if (audioBitrate > 0) {
      _sdp = setMediaBitrate(setMediaBitrate(sdp, 'video', videoBitrate), 'audio', audioBitrate);
    }
    else {
      _sdp = setMediaBitrate(sdp, 'video', videoBitrate);
    }
  }
  else {
    if (audioBitrate > 0) {
      _sdp = setMediaBitrate(sdp, 'audio', audioBitrate);
    }
    else {
      _sdp = sdp;
    }
...
function getCodecId(sdp){
  const regex = /^m=video*.+((UDP\/)?(TLS\/)?(RTP\/)?SAVPF).+/gm;
  var matches = sdp.match(regex);
  var str = matches && matches.length ? matches[0] : '';
  const codeId = str.split("SAVPF ")[1].substring(0,str.split("SAVPF ")[1].indexOf(" "));
  return codeId;
}
...
function setMediaBitrate(sdp, media, bitrate) {
    ...
    if (lines[line].indexOf('b') === 0) {
        ...
        lines[line] = 'b=' + bitrateFlag + ':' + bitrate;
        return lines.join('\n');
    }
    ...
    newLines.push('b=' + bitrateFlag + ':' + bitrate);
   ...
   */
  this.processAnswer = function (sdpAnswer, bandwidthOptions, callback) {
    callback = (callback || noop).bind(this)

    var answer = new RTCSessionDescription({
      type: 'answer',
      sdp: setMediaBitrates(sdpAnswer, bandwidthOptions.videoBitrate, bandwidthOptions.audioBitrate, bandwidthOptions.googlemax, bandwidthOptions.googlemin)
    })
